...
def load_monitor_csv(path: Path) -> pd.DataFrame:
    """
    Robust loader for SB3 Monitor CSV files.

    Tries standard parsing first, then falls back to python engine and skips bad lines.
    Keeps only the columns we actually need.
    """
    try:
    ...
    wanted = [c for c in METRICS if c in df.columns]
    if not wanted:
        raise ValueError(
            f"Could not find any expected monitor columns in {path}. "
...
def meaningful_difference_checks(seen: Dict[str, Any], unseen: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    add_check(
        ...
        abs(float(seen.get("violation_count_mean", 0.0)) - float(unseen.get("violation_count_mean", 0.0))) > 1e-9,
    )
    ...
    add_check(
        ...
        abs(float(seen.get("near_miss_count_mean", 0.0)) - float(unseen.get("near_miss_count_mean", 0.0))) > 1e-9,
    )
    ...
    add_check(
        ...
        abs(float(seen.get("shield_count_mean", 0.0)) - float(unseen.get("shield_count_mean", 0.0))) > 1e-9,
    )
