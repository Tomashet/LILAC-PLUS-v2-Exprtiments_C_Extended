...
def acquire_lock(run_dir: Path) -> bool:
    ...
    try:
        with lp.open("w", encoding="utf-8") as f:
            json.dump({"time": time.time(), "run_dir": str(run_dir)}, f)
        return True
    except Exception:
        return False
...
def preflight() -> List[RunSpec]:
    ...
    safe_print("=" * 90)
    safe_print("STAGE S-EXTENSION LAUNCHER")
    safe_print("=" * 90)
...
def run_one(spec: RunSpec) -> Tuple[str, str]:
    ...
    if looks_completed(run_dir):
        write_jsonl(
            LAUNCHER_LOG,
            {"event": "skip_completed", "run_name": run_name, "run_dir": str(run_dir), "time": time.time()},
        )
    ...
    if lock_path(run_dir).exists():
        write_jsonl(
            LAUNCHER_LOG,
            {"event": "skip_locked", "run_name": run_name, "run_dir": str(run_dir), "time": time.time()},
        )
    ...
    if not acquire_lock(run_dir):
        write_jsonl(
            LAUNCHER_LOG,
            {"event": "skip_lock_acquire_failed", "run_name": run_name, "run_dir": str(run_dir), "time": time.time()},
        )
    ...
    try:
        with stdout_path(run_dir).open("a", encoding="utf-8") as out_f, stderr_path(run_dir).open("a", encoding="utf-8") as err_f:
            out_f.write("\n" + "=" * 80 + "\n")
        ...
        if rc == 0:
            ...
            write_jsonl(
                LAUNCHER_LOG,
                {"event": "success", "run_name": run_name, "run_dir": str(run_dir), "returncode": rc, "time": time.time()},
            )
        ...
        write_jsonl(
            LAUNCHER_LOG,
            {"event": "failure", "run_name": run_name, "run_dir": str(run_dir), "returncode": rc, "time": time.time()},
        )
...
def main(max_workers: int = DEFAULT_MAX_WORKERS) -> None:
    ...
    safe_print("\n" + "=" * 90)
    safe_print("STAGE S-EXTENSION LAUNCH SUMMARY")
    safe_print("=" * 90)
    for k, v in counts.items():
        safe_print(f"{k:>22}: {v}")
    ...
    write_jsonl(
        LAUNCHER_LOG,
        {"event": "summary", "counts": counts, "elapsed_sec": elapsed, "workers": max_workers, "time": time.time()},
    )
